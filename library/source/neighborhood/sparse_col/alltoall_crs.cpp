                              ...
                              MPIL_Comm* comm)
{
    ...
    if (!(xinfo->crs_num_initialized))
    {
        ...
        MPI_Allreduce(
            MPI_IN_PLACE, msg_counts, num_procs, MPI_INT, MPI_SUM, comm->global_comm);
    ...
    {
        proc = dest[i];
        MPI_Isend(&(send_buffer[i * send_bytes]),
                  send_bytes,
                  MPI_BYTE,
                  proc,
                  tag,
    ...
    while (ctr < *recv_nnz)
    {
        ...
        src[ctr] = proc;
        MPI_Recv(&(recv_buffer[ctr * send_bytes]),
                 send_bytes,
                 MPI_BYTE,
                 proc,
                 tag,
                             ...
                             MPIL_Comm* comm)
{
    ...
    {
        proc = dest[i];
        MPI_Issend(&(send_buffer[i * send_bytes]),
                   send_bytes,
                   MPI_BYTE,
                   proc,
                   tag,
    ...
    while (1)
    {
        ...
        if (flag)
        {
            ...
            MPI_Recv(&(recv_buffer[ctr * recv_bytes]),
                     recv_bytes,
                     MPI_BYTE,
                     proc,
                     tag,
...
void local_redistribute(int node_recv_size,
