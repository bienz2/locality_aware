 ...
 *		<br>MULTILEADER: Group of leaders aggregates message among themselves
 * before distribution. <br>LOCALITY_AWARE|LOCALITY|LOC: Uses hardware architecture based
 * *communication tree for operations. <br>NODE_AWARE: Partial locality awareness (limited
 * to node level locality) <br>BATCH: Aggregate messages for bulk send <br>ASYNC: Uses RMA
 * for one-sided communication <br>GPU: Offload communication to GPU <br>CTC: Copy
 * messages from gpu to cpu before sending. <br>INIT: Persistent Communication used
 * internally. <br>PMPI: Calls the underlying MPI implementation.
 **/
...
/** @brief Enumeration of implemented alltoallv compressed row storage algorithms @ingroup
 * alg_enum**/
enum AlltoallvCRSMethod
{
    ...
    ALLTOALLV_CRS_PERSONALIZED_DENSE,
 ...
 * If an invalid value is provided, called MPIL functions will use their default
 * algorithm.
 *
 * @{
 */
 ...
 * @param [in] The MPI Communicator for the libray to use as the "world" communicator
 */
int MPIL_Init(MPI_Comm);

/**@brief Finalize and free any resources made by the locality_aware library. */
int MPIL_Finalize();
 ...
 * @param [out] xcomm newly initialized MPIL_Comm object.
 ...
 * @details
 *     Sets up LocalityComm by  gathering arrays for get_node, get_local, and get_global
methods.
...
/** @brief Generates MPIL_Topo from using comm object returned from
...
/** @brief Subdivides an MPIL_Comm into subgroups and assigns communication leader for
each subgroup.
 * @details
 *     Global communicator is split into sub-communicators of maximum size
proc_per_leader.
 *     Calls MPIL_Comm_topo_init if MPIL_Comm::local_comm is not set.
 *     Leader is selected from each sub-communicator and added to
MPIL_Comm::leader_group_comm
 *
...
/** @brief Wrapper around MPI_Neighbor_alltoallv that accepts an already generated
 * topology.
 *	@ingroup collective_func
 */
...
/** @brief Extended version of neighbor alltoallv that allows you to provide global
 * indices.
 *	@ingroup collective_func
 */
...
/** @brief Wrapper around persistent version MPI_Neighbor_alltoallv that accepts an
 * already generated topology object.
 *	@ingroup collective_func
 */
...
/** @brief Extended version of MPIL_Neighbor_alltoallv_init_topo that allows you to
 * provide global indices.
 *	@ingroup collective_func
 */
...
/** @brief Dynamically allocates enough space for char[bytes] and returns pointer to the
 * new allocation.
 * @details
